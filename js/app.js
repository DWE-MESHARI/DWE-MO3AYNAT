...
function renderLogin(){
  ...
  document.getElementById('app').innerHTML = `
    <div class="admin-lock">
      <div class="admin-lock-dots"></div>
      <div class="admin-lock-skyline">
        ${[36,58,44,70,50,64,38].map(h=>`<div class="b" style="height:${h}px;width:30px;"></div>`).join('')}
      </div>
      <div class="lock-box lock-box-v2">
        <div class="lock-topbar"></div>
        <div class="brand-mark" style="width:80px;height:80px; margin:-42px auto 16px; box-shadow:var(--shadow-3); border:4px solid var(--paper);"><img src="${LOGO_SRC}" alt="دار وإعمار"></div>
        <h3>تسجيل دخول الموظفين</h3>
        <p>${usingFirebaseAuth? 'هذه الصفحة مخصصة لموظفي دار وإعمار فقط' : 'وضع تجريبي — البيانات التجريبية موضّحة أدناه (سيُستبدل بحسابات موظفين حقيقية عند ربط Firebase)'}</p>
        <form id="loginform" style="margin-top:22px;">
          <div class="field input-icon" style="text-align:right;">
            <span class="input-icon-mark">${ICON.mail}</span>
            <input type="email" id="login-email" placeholder="البريد الإلكتروني" autocomplete="username" style="text-align:right;">
          </div>
          <div class="field input-icon" style="text-align:right; margin-bottom:0;">
            <span class="input-icon-mark">${ICON.lock}</span>
            <input type="password" id="login-password" placeholder="كلمة المرور" autocomplete="current-password" style="text-align:right;">
            <button type="button" class="pw-toggle" onclick="togglePwVisibility()" aria-label="إظهار كلمة المرور">${ICON.eye}</button>
          </div>
          <div class="lock-err" id="lock-err"></div>
          <button type="submit" class="btn btn-primary btn-block" style="margin-top:16px;">${ICON.lock}<span>دخول</span></button>
          <button type="button" class="btn btn-ghost btn-block" style="margin-top:10px;" onclick="goto('guest')">رجوع لصفحة الضيوف</button>
        </form>
